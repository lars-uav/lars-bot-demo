# Image Blending of Calibration Target on Video Input using OpenCV (Remove Background and Overlay)


'''
Receive the Video Input (UDP Stream) from the Drone Camera
Initialize the Calibration Grid Image (Blank Image with Grid Lines)
Capture Overlay the Calibration Grid Image on the Video Input
Display a Report Panel (on the Right Hand Side) with Per Grid NDVI Value (and Stressed and Non-Stressed)
! Stressed Grid (NDVI between [-0.5 and .1] and Non-Stressed Grid (NDVI between [0.1 and 1])
Display a Red Border (around the Stressed Grid Cell) and Give the Grid Cell Index (for the Ground Bot)
'''
# Importing Libraries
import time
import os
...
class GridOverlay:
    ...
    def add_status_text(self, frame):
            ...
            if self.state in self.status_messages:
                ...
                font_scale = 1.2  # Reduced from 2 to 1.2
                thickness = 2     # Reduced from 3 to 2
                text_size = cv2.getTextSize(message, font, font_scale, thickness)[0]
                text_x = (frame.shape[1] - text_size[0]) // 2
                text_y = 70  # Reduced from 100 to 70 to match smaller font
    ...
    def run(self):
        ...
        # Set capture resolution (if supported by the camera)
        ...
        while True:
            ...
            # Resize frame to desired display dimensions
            frame = self.resize_frame(frame)

            # Update state based on timing
            self.update_state()
            
            # End sequence if we've shown all grids
            if self.state == GridState.END:
                break

            # Get and overlay current grid
